...
# Add obstacles
# Block region (upper left)
for r in range(3, 10):
    for c in range(3, 12):
        grid.set_obstacle(r, c)

# Horizontal bar across row 12 
for c in range(0, 35):
    grid.set_obstacle(12, c)

# Vertical bar at column 15
for r in range(5, 30):
    grid.set_obstacle(r, 15)
...
for c in range(5, 20):
    grid.set_obstacle(36, c)

# Multi-column maze-like pillars 
for r in range(2, 38, 2):
    grid.set_obstacle(r, 28)
    grid.set_obstacle(r, 30)
    grid.set_obstacle(r, 32)

# A thick horizontal band
for c in range(10, 30):
    grid.set_obstacle(20, c)

# Another obstacle block in lower right
for r in range(32, 37):
    for c in range(32, 37):
        grid.set_obstacle(r, c)

# Curved diagonal (slanted wall)
for i in range(10, 30):
    grid.set_obstacle(i, 39 - i // 2)
